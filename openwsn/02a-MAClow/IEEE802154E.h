...
typedef enum {
   ...
   /* piggy201: NF states */
   S_NFPROBEOFFSET           = 0x1a,   // waiting to prepare NF probe
   S_NPPREPARE               = 0x1b,   // preparing
   S_NPREADY                 = 0x1c,   // ok to probe
   S_NPSTART                 = 0x1d,   // go
} ieee154e_state_t;
...
void     activity_np1();  //enable Rx for probing
void     activity_npe1();
void     activity_np2();  //probe
void     activity_np3(uint16_t);  //blacklist update from local NF probe

uint8_t   nextAvail(uint8_t);
void     applyTxMask();
void     applyRxMask();

void     kidsUpdate(OpenQueueEntry_t* pkt);
