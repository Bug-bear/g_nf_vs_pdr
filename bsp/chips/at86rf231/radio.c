...
typedef struct {
   ...
   radiotimer_capture_cbt    ED_cb; //piggy103a: callback handler for ED
   radio_state_t             state;
} radio_vars_t;
...
void radio_init() {
   ...
   radio_spiWriteReg(RG_IRQ_MASK,
                     (AT_IRQ_RX_START| AT_IRQ_TRX_END | AT_IRQ_CCA_ED_DONE));  // tell radio to fire interrupt on TRX_END and RX_START
...
//piggy103a: set callback handler for ED
void radio_setEDCb(radiotimer_capture_cbt cb){
   radio_vars.ED_cb    = cb;
}
...
uint8_t radio_isr() {
   ...
   if (irq_status & AT_IRQ_CCA_ED_DONE) {
      // callback version
      if (radio_vars.ED_cb!=NULL) {
        // change state
        radio_vars.state = RADIOSTATE_ED_DONE;
        radio_vars.ED_cb(capturedTime); //piggy21b
         // kick the OS
        return 1;
      }
   }
